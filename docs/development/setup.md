...
3. ビルドする（初回は ESP-IDF がワークスペース配下に自動取得される）。
   ...
   **注意**: `components/esp_hid_gap/`（vendorしたコンポーネント）の`.c`/`.h`を
   編集した場合、Cargoの依存関係追跡が変更を検知できず、インクリメンタルビルドでは
   古いコンパイル結果・bindgen生成物のまま（変更が反映されない）になることがある
   （実機で「追加した関数が見つからない」というリンクエラーで発覚）。
   その場合は`cargo clean -p esp-idf-sys`を実行してからビルドし直すこと
   （ESP-IDF全体の再ビルドが走るため数分かかる）。
