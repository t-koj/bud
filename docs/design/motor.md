...
* 実機（基板接続済み）での動作確認で、`set_servo_angle`が`ESP_FAIL`で失敗し続ける
  現象を確認した。原因未特定（配線/アドレス不一致、I2Cバスのタイミング、
  ATOMIC Motionベース側の応答性等の可能性がある）。切り分けのため、失敗ログに
  角度の値を追加し、失敗から成功に復帰した際にも`set_servo_angle(...) recovered`
  をログ出力するようにした（成功が続いている間はログを出さない。詳細は
  [led.md](led.md)参照）。
