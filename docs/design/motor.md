...
`main.rs`では失敗したチャンネルを以降二度と再試行しない（現在はサーボ2ch分を
`servo_ok`配列で管理）。ATOMIC Motionベースは起動時の配線の有無で決まり実行中に
後から繋がることは無いため、初回の書き込みで確立しなければ以降も回復する見込みが無く、
毎フレーム（間引いた定期的な再試行でも同様）試行し続けるとメインループの実効周期が
`LOOP_INTERVAL_MS`から大きく後退し、PS4コントローラーの入力ポーリング
（`gamepad.poll()`の呼び出し頻度）が落ちる実機不具合を確認した。
