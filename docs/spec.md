...
* S1/S3（180度サーボ）は目標角度をそのまま送るが、S2/S4（360度連続回転サーボ）は
  急激な角度変化を送るとサーボが一定周期（体感1秒程度）でしか反応しなくなる現象が
  実機で観測されたため、1フレームあたりの角度変化量を制限してなめらかに追従させる
  （`SERVO_ANGLE_STEP_LIMIT_DEG`, 未調整）。詳細は[design/motor.md](design/motor.md)参照。
* S2/S4に接続する360度連続回転サーボは、停止点（ニュートラル点）に個体差があり
  90度ちょうどとは限らない。ずれていると停止のつもりでも微回転し続けるため、
  チャンネルごとのトリム値`SERVO_NEUTRAL_TRIM_DEG`（`main.rs`）で実機校正する
  必要がある（現状は未校正でデフォルト0.0）。詳細は[design/motor.md](design/motor.md)参照。
