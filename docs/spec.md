...
* S1/S3（180度サーボ）は角度レジスタ、S2/S4（360度連続回転サーボ）はPWMパルス幅
  レジスタで制御する。角度レジスタ経由の制御では、大きな角度変化時に実機で約1秒の
  反映遅延が観測されたため、S2/S4はスムージングの影響を受けないPWMパルス幅
  レジスタに切り替えている。詳細は[design/motor.md](design/motor.md)参照。
* S2/S4に接続する360度連続回転サーボは、停止点（ニュートラル点）に個体差があり
  1500us（角度でいう90度相当）ちょうどとは限らない。ずれていると停止のつもりでも
  微回転し続けるため、チャンネルごとのトリム値`SERVO_NEUTRAL_TRIM_US`（`main.rs`）
  で実機校正する必要がある（現状は未校正でデフォルト0）。詳細は
  [design/motor.md](design/motor.md)参照。
